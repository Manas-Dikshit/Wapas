...
import { currentProfile, notifications } from '@/lib/mock-data';
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[248px] flex-col border-r border-navy-100/70 bg-white lg:flex">
        ...
        <div className="border-t border-navy-100/70 p-4">
          <Link href="/profile" className="flex items-center gap-3 rounded-2xl p-2 hover:bg-navy-50">
            <Avatar name={currentProfile.fullName} />
            <div className="min-w-0">
              <p className="truncate text-sm font-bold text-navy-600">{currentProfile.fullName}</p>
              <p className="truncate text-xs text-navy-400">{currentProfile.companyName}</p>
            </div>
          </Link>
        </div>
      </aside>
          ...
          <div className="fixed inset-0 z-40 bg-navy-700/40 backdrop-blur-sm lg:hidden" onClick={() => setMobileMenuOpen(false)}>
            <div className="absolute right-0 top-0 h-full w-72 animate-fade-up bg-white p-5 shadow-floating" onClick={(e) => e.stopPropagation()}>
              <div className="mb-6 flex items-center gap-3">
                <Avatar name={currentProfile.fullName} />
                <div>
                  <p className="text-sm font-bold text-navy-600">{currentProfile.fullName}</p>
                  <p className="text-xs text-navy-400">{currentProfile.companyName}</p>
                </div>
              </div>
